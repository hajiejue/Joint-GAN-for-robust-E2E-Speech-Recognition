...
class SequenceWise(torch.nn.Module):
    def __init__(self, module):
        ...
        """
        super(SequenceWise, self).__init__() # 标准写法
        self.module = module

    def forward(self, x):
        n, t = x.size(0), x.size(1)         #取维数
        x = x.view(n * t, -1)               #给重新定义维数
        x = self.module(x)
        x = x.view(n, t, -1)
        return x
...
class EnhanceModel(ModelBase):
    ...
    #def forward(self,clean_inputs,mix_inputs,mix_log_inputs,cos_angles,input_sizes ):
        '''Encoder forward
        :param xs:
        :param ilens:
        :return:
        ...
        out = out.masked_fill(mask.bool(), 0)
