...
class MixSequentialDataset(Dataset, FbankFeatLabelParser):
    def __init__(self, args, data_dir, dict_file):
        ...
        if self.feat_type.split('_')[0] == 'kaldi':
            if data_dir == '/usr/home/wudamu/FP/kaldi-trunk/egs/aishell/s5/data/train_new':
                self.data_dir_clean = '/usr/home/wudamu/FP/kaldi-trunk/egs/aishell/s5/data/train'
                #self.clean_feat_scp = os.path.join(data_dir, 'clean_feats.scp')
                #self.clean_angle_scp = os.path.join(data_dir, 'clean_angles.scp')
                self.clean_feat_scp = os.path.join(self.data_dir_clean, 'clean_feats.scp')
                self.clean_angle_scp = os.path.join(self.data_dir_clean, 'clean_angles.scp')
                self.mix_feat_scp = os.path.join(data_dir, 'clean_feats.scp')
                self.mix_angle_scp = os.path.join(data_dir, 'clean_angles.scp')
                self.mix_feat_len_scp = os.path.join(data_dir, 'kaldi_feat_len.scp')
            else:
                self.data_dir_clean = '/usr/home/wudamu/FP/kaldi-trunk/egs/aishell/s5/data/dev'
                #self.clean_feat_scp = os.path.join(data_dir, 'clean_feats.scp')
                #self.clean_angle_scp = os.path.join(data_dir, 'clean_angles.scp')
                self.clean_feat_scp = os.path.join(self.data_dir_clean, 'clean_feats.scp')
                self.clean_angle_scp = os.path.join(self.data_dir_clean, 'clean_angles.scp')
        ...
        if self.model_unit == 'char':                 
            self.label_file = os.path.join(data_dir, 'text')
