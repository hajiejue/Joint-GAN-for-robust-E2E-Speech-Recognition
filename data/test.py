    ...
    for num in range(repeat_num):
        for tmp in wav_path_list:
            ...
            if y is not None:
                ...
                # D = librosa.stft(y, n_fft=512, hop_length=256, win_length=512, window='hann')
                spect = np.abs(D)
                angle = np.angle(D)
...
def main():
    data_dir = "/usr/home/wudamu/FP/kaldi-trunk/egs/aishell/s5/data/dev"
    feat_dir = "/usr/home/wudamu/FP/kaldi-trunk/egs/aishell/s5/data/dev"
    noise_repeat_num = 1
    ...
    with open(clean_wav_scp, 'r',
              encoding='utf-8') as fid:  # 'BAC009S0002W0123 /usr/home/wudamu/FP/Download/data_aishell/wav/S0002/train/S0002/BAC009S0002W0123.wav\n'
        for line in fid:
            line = line.strip().replace('\n', '')
            uttid, wav_path = line.split()
            clean_wav_list.append((uttid,
                                   wav_path))  # [('BAC009S0002W0122', '/usr/home/wudamu/FP/Download/data_aishell/wav/S0002/train/S0002/BAC009S0002W0122.wav')]
    print('clean_wav_list', len(clean_wav_list))
    noise_wav_list = []
    # noise_wav_scp = os.path.join(data_dir, 'noise/noise.scp')
    noise_wav_scp = "/usr/home/wudamu/FP/kaldi-trunk/egs/aishell/s5/data/noise/noise.scp"
    with open(noise_wav_scp, 'r', encoding='utf-8') as pp:
        for line_noise in pp:
            line_noise = line_noise.strip().replace('\n', '')
            uttid, wav_path = line_noise.split()
    ...
    # make_feature (clean_wav_list, noise_wav_list, clean_feat_dir, 1, False)
