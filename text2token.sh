...
embed_init_file=/usr/home/wudamu/FP/words_vectors
...
if [ ${stage} -le 3 ]; then
    ...
    mkdir -p ${lmdatadir}
    text2token.py -s 1 -n 1 ${dataroot}/train/text | cut -f 2- -d" " | perl -pe 's/\n/ <eos> /g' \
    ...
    text2token.py -s 1 -n 1 ${dataroot}/${train_dev}/text | cut -f 2- -d" " | perl -pe 's/\n/ <eos> /g' \
        > ${lmdatadir}/valid.txt
